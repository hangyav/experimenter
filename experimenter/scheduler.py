...
class LocalScheduler():
    ...
    def __init__(self, num_processes=1, wait_for_unfinished=True):
    ...
    def execute(self, nodes):
        ...
        if self.num_processes == 1:
            self._execute_single_process(nodes)
            return
    ...
    def _execute_single_process(self, nodes):
        while not nodes.issubset(self.finished_nodes):
            runnable_nodes = set()
            for node in nodes:
                runnable_nodes.update(self._get_runnable_nodes(node))
